...
Router.onBeforeAction(requireAdmin, {only: ['manageHome','postSubmit','npostSubmit','users','uploadFiles']});
Router.onBeforeAction(requireLogin, {only: ['manageHome','postSubmit','npostSubmit','userProfile']});

Router.route('/', {
    name: 'home',
    waitOn: function() {
        return [Meteor.subscribe('HomeNewsPosts'),Meteor.subscribe('HomeEditorOpinionPosts'),Meteor.subscribe('GSTPost'), Meteor.subscribe('top5Posts')];
    },
    data: function() {
...
Router.route('/adminsignin', {name: 'adminsignin'});  //login: hydaudit3@gmail.com;cbecaudit
...
DPostsListController = RouteController.extend({ template: 'dpostsList',
    ...
    dpostsEditorSearchParams: function() {
        if(this.params.dpostsSearch){
            return {
                eOpinion: true,
                $or: [
    ...
    dMyposts: function() {
        return DPosts.find(this.dMypostsSearchParams(),this.findOptions());
    },
    dpostsEditor: function() {
        return DPosts.find(this.dpostsEditorSearchParams(),this.findOptions());
    },
    data: function() {
        ...
        return {
            dposts: this.dposts(),
            dMyposts: this.dMyposts(),
            dpostsEditor: this.dpostsEditor(),
...
Router.route('/uploadFiles', {
    name: 'uploadFiles',
    waitOn: function() {
        return Meteor.subscribe('filescfs');
    }
});


//Search related
Router.route('/search/:searchLimit?/:searchText?', {
    name: 'search'
    ,waitOn: function() {
        Session.set("showSpinner", true);
        Meteor.call("globalSearch", this.params.searchText, this.params.searchLimit, function (err, res) {
